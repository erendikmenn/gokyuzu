...
- Altyapı: AWS S3 + CloudFront (hesap "gokyuzu-admin", eu-central-1), sertifikalar ACM (us-east-1), DNS Cloudflare (erenailab.com). Canlı `fs.erenailab.com` Cloudflare proxy'si arkasında (ana sayfada tarayıcı doğrulaması, güvenlik başlıkları, istek sınırı; tarayıcı önbelleği kaynaktaki sürelere uyar); staging "DNS only". Canlı yayın, CloudFront tazelemesi bitince Cloudflare önbelleğini de temizler.
- Kullanım istatistiği: `.venv/bin/python tools/analytics/report.py` (canlı, son 7 gün; `staging`, `--days N`). Anonimdir, kayıtlar 30 gün sonra silinir (CONTRACTS-SF.md §11).
