...
Inputs: cached 3DEP mosaics (data/sf/raw/3dep) and data/sf/cache/terrain/water_final.pkl (terrain_build.py).
...
from terrain_common import RAW, CACHE, OUT, ROOT_MIN_X, ROOT_MIN_Z, CORE
...
def main():
    ...
    depth = np.where(sea, np.maximum(-h, 0.0), 0.0)
    est = np.clip(dist * 0.012, 0.0, 60.0)                      # offshore where the DEM has no bathymetry (0)
